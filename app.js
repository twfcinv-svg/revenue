/* app.js — v3.1 修正標籤重疊：
 1) Treemap 葉節點標籤改為「2 行區塊（名稱/數值）+ 動態字級 + clipPath」
 2) 量測僅在渲染完成後進行一次 getBBox（批次），避免反覆 reflow。
 3) 保留 v2 的效能優化（Links 預索引、分幀渲染）；僅在標籤調整階段使用最少量量測。
*/
...
const COL_MAP = {}; // COL_MAP['202508'] = { YoY: '202508單月合併營收年成長(%)', MoM: '...' }

let revenueRows = [], linksRows = [], months = [];
let byCode = new Map();
let byName = new Map();
let linksByUp = new Map();   // key: 上游代號
...
function setupDownloadButton(){
  // 隱藏舊的 dlData（若 index 有放）
  const old = document.getElementById('dlData');
  if (old) old.style.display = 'none';

  // 新按鈕（固定在右上角）
  const a = document.createElement('a');
  a.href = 'data.xlsx?v='+URL_VER;
  a.textContent = '下載 data.xlsx';
  ...
  Object.assign(a.style, {
    ...
    boxShadow: '0 1px 2px rgba(0,0,0,.25)', border: '1px solid rgba(15,23,42,.15)',
...
const LabelFit = {
  ...
  fitBlock(textEl, w, h){
    ...
    textEl.setAttribute('font-size', this.maxFont);
    ...
    const finalSize = Math.floor(this.maxFont * scale);
...
function renderTreemap(svgId, hintId, edges, codeField, month, metric, colorMode){
  ...
  labels.each(function(d){
    ...
    t2.setAttribute('x','0');
    t2.setAttribute('dy', `${LabelFit.lineHeight}em`);
    t2.textContent = vstr;

    this.appendChild(t1);
    this.appendChild(t2);

    // title 作為 tooltip 顯示完整資訊
    const title = document.createElementNS('http://www.w3.org/2000/svg','title');
    title.textContent = `${name} ${vstr}`;
