...
/* app.js — 診斷 + 不快取版 */
const PAGE_VER = new URLSearchParams(location.search).get('v') || Date.now();
const XLSX_FILE = new URL('data.xlsx?v='+PAGE_VER, location.href).toString();
...
window.addEventListener('DOMContentLoaded', async()=>{
  setStatus('載入資料中…（VER='+PAGE_VER+'）');
  try{ await loadWorkbook(); initControls(); setStatus(`資料就緒（偵測月份：${months.join(', ')||'無'}｜VER=${PAGE_VER}）`);}catch(e){ console.error(e); setStatus('載入失敗：'+e.message);} 
...
async function loadWorkbook(){
  ...
  console.log('[診斷] Revenue 首列欄名：', headers);
  ...
  if(months.length===0){ setStatus('偵測月份=0；請打開 Console 檢視 [診斷] 欄名，確認月份欄名是否為 202512單月合併營收年成長(%) / 月變動(%) 等格式'); }
}
...
function handleRun(){
  ...
  if(!codeKey){ setStatus('請輸入股票代號或公司名稱'); return; }
  if(!byCode.has(codeKey)){ const hit=revenueRows.find(r=>z(r['名稱']).includes(z(raw).trim())); if(hit){ codeKey=normCode(hit['個股']??hit['代號']); } }
  if(!byCode.has(codeKey)){ const sample=Array.from(byCode.keys()).slice(0,10).join(', '); setStatus(`找不到此代號/名稱。可用代號前10：${sample}`); return; }
  ...
  const upStocksWithVal=upstreamEdges.map(r=>normCode(r['上游代號'])).map(k=>byCode.get(k)).filter(row=>row&&getMetricValue(row,month,metric)!=null).length;
  const downStocksWithVal=downstreamEdges.map(r=>normCode(r['下游代號'])).map(k=>byCode.get(k)).filter(row=>row&&getMetricValue(row,month,metric)!=null).length;

  const rowSelf=byCode.get(codeKey); const selfVal=getMetricValue(rowSelf,month,metric);
  setStatus(`查詢：輸入='${raw}' → 代號='${z(rowSelf['個股'])}' ｜ 上游邊=${upstreamEdges.length}（有值=${upStocksWithVal}），下游邊=${downstreamEdges.length}（有值=${downStocksWithVal}）｜本身=${displayPct(selfVal)}｜VER=${PAGE_VER}`);
...
function renderTreemap(svgId, hintId, edges, codeField, month, metric, colorMode){ const svg=d3.select('#'+svgId); svg.selectAll('*').remove(); const wrap=svg.node().parentElement; const W=wrap.clientWidth-16; const H=parseInt(getComputedStyle(svg.node()).height)||560; svg.attr('width',W).attr('height',H); const groups=new Map(); for(const e of edges){ const rel=normText(e['關係類型']||'未分類'); const key=normCode(e[codeField]); const r=byCode.get(key); if(!r) continue; const v=getMetricValue(r,month,metric); if(v==null) continue; if(!groups.has(rel)) groups.set(rel,[]); groups.get(rel).push({ code:r['個股'], name:r['名稱'], value:v }); } const hint=document.getElementById(hintId); if(groups.size===0){ hint.textContent='此區在選定月份沒有可用數據'; return; } else { hint.textContent=''; } const children=[]; for(const [rel,list] of groups){ const avg=d3.mean(list,d=>d.value); const kids=list.map(s=>({ name:`${s.code} ${s.name||''}`.trim(), code:s.code, value:Math.max(0.01,Math.abs(s.value)), raw:s.value })); children.push({ name:rel, avg, children:kids }); } const root=d3.hierarchy({ children }).sum(d=>d.value).sort((a,b)=>(b.value||0)-(a.value||0)); d3.treemap().size([W,H]).paddingOuter(8).paddingInner(3)(root); const g=svg.append('g'); const parents=g.selectAll('g.parent').data(root.children||[]).enter().append('g').attr('class','parent'); parents.append('rect').attr('class','group-border').attr('x',d=>d.x0).attr('y',d=>d.y0).attr('width',d=>Math.max(0,d.x1-d.x0)).attr('height',d=>Math.max(0,d.y1-d.y0)); parents.append('text').attr('class','node-title').attr('x',d=>d.x0+6).attr('y',d=>d.y0+16).text(d=>`${d.data.name}  平均：${displayPct(d.data.avg)}`); const node=g.selectAll('g.node').data(root.leaves()).enter().append('g').attr('class','node').attr('transform',d=>`translate(${d.x0},${d.y0})`); node.append('rect').attr('class','node-rect').attr('width',d=>Math.max(0,d.x1-d.x0)).attr('height',d=>Math.max(0,d.y1-d.y0)).attr('fill', d=>` ${''}` || colorFor(d.data.raw,colorMode)); node.append('text').attr('class','node-sub').attr('x',6).attr('y',16).text(d=>`${(d.data.code||'').toString().slice(0,6)} ${displayPct(d.data.raw)}`).each(function(d){ const w=d.x1-d.x0; if(this.getBBox().width>w-8){ d3.select(this).attr('opacity',0.85).attr('font-size',10);} }); node.append('text').attr('class','node-sub').attr('x',6).attr('y',30).text(d=>`${safe((d.data.name||'').slice(0,8))}`).each(function(d){ const w=d.x1-d.x0; if(this.getBBox().width>w-8){ d3.select(this).attr('opacity',0.8).attr('font-size',10);} }); }
