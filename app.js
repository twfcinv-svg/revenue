...
function toHalfWidth(str){
  return z(str).replace(/[０-９Ａ-Ｚａ-ｚ]/g,
...
async function loadWorkbook(){
  const res = await fetch(XLSX_FILE); if(!res.ok) throw new Error(`讀取 data.xlsx 失敗（HTTP ${res.status}）。請確認 data.xlsx 與 index.html 同層。`);
  ...
  for(const h of headers){
    const m1 = h.match(/^(\d{6})\s*單月合併營收\s*年成長\s*[\(（]?\s*(?:%|％)\s*[\)）]?\s*$/);
    if(m1) found.add(m1[1]);
    const m2 = h.match(/^(\d{6})\s*單月合併營收\s*月變動\s*[\(（]?\s*(?:%|％)\s*[\)）]?\s*$/);
...
function initControls(){
  const sel = document.querySelector('#monthSelect'); if(!sel) return; sel.innerHTML='';
...
function getMetricValue(row, month, metric){
  if(!row) return null; const part=COL_SUFFIX[metric];
  const patterns=[`${month}單月合併營收${part}(%)`, `${month}單月合併營收${part}(％)`, `${month}單月合併營收${part}（%）`, `${month}單月合併營收${part}（％）`];
  let v=null; for(const p of patterns){ if(row[p]!=null && row[p]!=='' ){ v=row[p]; break; } } if(v==null) return null; v = Number(v); return Number.isFinite(v) ? v : null;
}
...
function handleRun(){
  ...
  if(!byCode.has(codeKey)){
    const hit = revenueRows.find(r => z(r['名稱']).includes(z(raw).trim()));
    if(hit){ codeKey = normCode(hit['個股'] ?? hit['代號']); }
  }

  if(!byCode.has(codeKey)){
    const sample = Array.from(byCode.keys()).slice(0,10).join(', ');
    setStatus(`找不到此代號/名稱。「可用代號前10筆」：${sample} …`);
    return;
  }
  ...
  const selfVal = getMetricValue(rowSelf, month, metric);
  setStatus(`查詢：輸入='${raw}' → 代號='${z(rowSelf['個股'])}' ｜ 上游邊=${upstreamEdges.length}（有值=${upStocksWithVal}），下游邊=${downstreamEdges.length}（有值=${downStocksWithVal}）｜本身=${displayPct(selfVal)}`);
...
function renderResultChip(selfRow, month, metric){
  const host=document.querySelector('#resultChip');
  const v=getMetricValue(selfRow, month, metric);
  const cls=v==null?'':(v>=0?'good':'bad');
  host.innerHTML=`
    <div class="result-card">
      <div class="row1"><strong>${safe(selfRow['個股'])}｜${safe(selfRow['名稱'])}</strong>
        <span>${month.slice(0,4)}/${month.slice(4,6)} / ${metric}</span></div>
      <div class="row2"><span>${safe(selfRow['產業別']||'')}</span><span class="${cls}">${displayPct(v)}</span></div>
    </div>`;
}

function renderTreemap(svgId, hintId, edges, codeField, month, metric, colorMode){
  const svg=d3.select('#'+svgId); svg.selectAll('*').remove();
  const wrap=svg.node().parentElement; const W=wrap.clientWidth-16; const H=parseInt(getComputedStyle(svg.node()).height)||560; svg.attr('width',W).attr('height',H);

  const groups=new Map();
  for(const e of edges){
    const rel=normText(e['關係類型']||'未分類');
    const key=normCode(e[codeField]);
    const r=byCode.get(key); if(!r) continue;
    const v=getMetricValue(r, month, metric); if(v==null) continue;
    if(!groups.has(rel)) groups.set(rel, []);
    groups.get(rel).push({ code:r['個股'], name:r['名稱'], value:v });
  }
